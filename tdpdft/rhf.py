...
REAL_EIG_THRESHOLD = getattr(__config__, 'tdscf_rhf_TDPDFT_pick_eig_threshold', 1e-4)
...
def get_ab(mf, mo_energy=None, mo_coeff=None, mo_occ=None):
    r'''A and B matrices for TDPDFT response function.
...
def get_nto(tdobj, state=1, threshold=OUTPUT_THRESHOLD, verbose=None):
    ...
    Note in the TDHF/TDPDFT calculations, the excitation part (X) is
    interpreted as the CIS coefficients and normalized to 1. The de-excitation
    part (Y) is ignored.

    Args:
        tdobj : TDA, or TDHF, or TDPDFT object
    ...
    if(tdobj._scf.rew is not None): 
      orbo = mo_coeff[:,numpy.logical_and(tdobj_.scf.rew[0]<mo_energy,mo_energy<tdobj._scf.rew[1])]
    else:
      print('rew none in get_nto')
    orbv = mo_coeff[:,mo_occ==0]
    nocc = orbo.shape[1]
    nvir = orbv.shape[1]

    cis_t1 = tdobj.xy[state_id][0]
    # TDPDFT (X,Y) has X^2-Y^2=1.
...
# when the system has degenerated irreducible representations.
    ...
    if mol.symmetry:
        ...
        if(tdobj._scf.rew is not None): 
          o_sym = orbsym_in_d2h[numpy.logical_and(tdobj._scf.rew[0]<mo_energy,mo_energy<tdobj._scf.rew[1])]
        else:
          print('rew none in get_nto symm')
...
def analyze(tdobj, verbose=None):
    ...
    if(tdobj._scf.rew is not None): 
      mo_energy = tdobj._scf.mo_energy 
      occidx = numpy.where(numpy.logical_and(tdobj._scf.rew[0]<mo_energy,mo_energy<tdobj._scf.rew[1]))[0]
      nocc = len(occidx)
    else:
      print('rew none in analyze')
        ...
        if log.verbose >= logger.INFO:
            ...
            for o, v in zip(o_idx, v_idx):
                log.info('    %4d -> %-4d %12.5f',
                         o+MO_BASE, v+MO_BASE+noccandfroz, x[o,v])
...
def analyze_wfnsym(tdobj, x_sym, x):
    '''Guess the wfn symmetry of TDPDFT X amplitude'''
...
def _contract_multipole(tdobj, ints, hermi=True, xy=None):
    ...
    orbo = mo_coeff[:,mo_occ==2]
    if(tdobj._scf.rew is not None): 
      orbo = mo_coeff[:,numpy.logical_and(tdobj._scf.rew[0]<mo_energy,mo_energy<tdobj._scf.rew[1])]
    else:
      print('rew none in _contract_multipole') 
    orbv = mo_coeff[:,mo_occ==0]
...
def as_scanner(td):
    '''Generating a scanner/solver for TDA/TDHF/TDPDFT PES.

    The returned solver is a function. This function requires one argument
    "mol" as input and returns total TDA/TDHF/TDPDFT energy.

    The solver will automatically use the results of last calculation as the
    initial guess of the new calculation.  All parameters assigned in the
    TDA/TDPDFT and the underlying SCF objects (conv_tol, max_memory etc) are
...
def gen_tdhf_operation(mf, fock_ao=None, singlet=True, wfnsym=None):
    ...
    if(mf.rew is not None): 
      ...
      print('rew none in gen_tdhf_operation')
    viridx = numpy.where(mo_occ==0)[0]
    nocc = len(occidx)
    nvir = len(viridx)
    orbv = mo_coeff[:,viridx]
...
class TDHF(TDA):
    ...
    def kernel(self, x0=None, nstates=None):
        ...
        # BGJ modify for consistency 
        #nocc = (self._scf.mo_occ>0).sum()
        #nmo = self._scf.mo_occ.size
        #nvir = nmo - nocc
        occidx = numpy.where(self._scf.mo_occ==2)[0]
        if(self._scf.rew is not None): 
          occidx = numpy.where(numpy.logical_and(self._scf.rew[0]<self._scf.mo_energy,self._scf.mo_energy<self._scf.rew[1]))[0]
